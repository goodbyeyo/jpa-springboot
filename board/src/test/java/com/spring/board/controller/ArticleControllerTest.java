...
import com.spring.board.util.FormDataEncoder;
...
class ArticleControllerTest {
    ...
    void givenNothing_whenRequestingArticlePage_thenRedirectsToLoginPage() throws Exception {
        ...
        mvc.perform(get("/articles/" + articleId))
                ...
                // .andExpect(redirectedUrl()) // redirectedUrl 은 full url (http:://localhost..) 의미
    ...
    @WithMockUser
    @DisplayName("[view] [GET] 게시글 페이지 - 정상 호출, 인증된 사용 ")
    @Test
...
//        then(articleService).should().getArticleCount();
    ...
    // userDetailsServiceBeanName = "userDetailsService" Bean 유일하므로 생략 가능
    // wooTest -> TestSecurityConfig 에 등록된 userName
    @WithUserDetails(value = "wooTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)    // 테스트 실행 전에 실행
    ...
    @WithUserDetails(value = "wooTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    ...
    @WithUserDetails(value = "pwd", setupBefore = TestExecutionEvent.TEST_EXECUTION)
